...
/////////////////////////////////////////////////////////////////////////
#pragma data:code
const unsigned char TEXT35[96][5]={
...
const unsigned char FONT2[45][5]={0x26, 0x49, 0x49, 0x49, 0x32, // S
...
//#else
void glcd_line(char x1, char y1, char x2, char y2, char color)
//#endif
{
   int        dy, dx;
...
/////////////////////////////////////////////////////////////////////////
//#ifdef LARGE_LCD
//void glcd_rect(int x1, int y1, int x2, int y2, char fill, char color)
//#else
void glcd_rect(char x1, char y1, char x2, char y2, char fill, char color)
//#endif
{
   if(fill==1)
   {
...
/////////////////////////////////////////////////////////////////////////
//#ifdef LARGE_LCD
//void glcd_bar(int x1, int y1, int x2, int y2, char width, char color)
//#else
...
/////////////////////////////////////////////////////////////////////////
//#ifdef LARGE_LCD
//void glcd_circle(int x, int y, int radius, char fill, char color)
//#else
void glcd_circle(char x, char y, char radius, char fill, char color)
//#endif
{
   #ifdef LARGE_LCD
   signed int a, b, P;
...
void glcd_text35(char x, char y, char* textptr, char color)
{
   ...
   for(i=0; textptr[i] != '\0'; ++i, ++x) // Loop through the passed string
   {
      if((textptr[i] >= ' ') && (textptr[i] <= '~'))
         for(l=0;l<5;l++)
		   pixelData[l]=TEXT35[textptr[i]-' '][l];
      else
         for(l=0;l<5;l++)
		   pixelData[l]=TEXT35[0][l]; // Default to space
      ...
      for(j=3; j>0; j--, x++)             // Loop through character byte data
      {
         ...
         {
            if(CHKBIT(pixelData[k], j)) // Check if the pixel should be set
            {
...
//                color - ON or OFF
/////////////////////////////////////////////////////////////////////////
void glcd_text57(char x, char y, char* textptr, char size, char color)
{
   char j, k, l, m, n;                       // Loop counters
   char pixelData[5];                     // Stores character data

   for(; *textptr != '\0'; ++textptr, ++x)// Loop through the passed string
   {
      if(*textptr < 'S') // Checks if the letter is in the first font array
         for(n=0;n<5;n++)pixelData[n]=FONT[*textptr - ' '][n];
      else if(*textptr <= '~') // Check if the letter is in the second font array
         for(n=0;n<5;n++)pixelData[n]=FONT2[*textptr - 'S'][n];
      else
         for(n=0;n<5;n++)pixelData[n]=FONT[0][n];   // Default to space
      ...
      for(j=0; j<5; ++j, x+=size)         // Loop through character byte data
      {
         ...
         {
            if(CHKBIT(pixelData[j], k)) // Check if the pixel should be set
            {
